...
from sklearn.svm import SVC  # 支持向量机分类器（一种强大的分类算法）
# 下面是一些评价模型好坏的指标
...
# note that support_vector_machine is excellent tool for binary classification: 
...
# PCA特征选择配置
PCA_COMPONENTS = 256  # 从768维BERT向量降到256维（保留主要信息，加速训练）
...
for fold, (train_idx, test_idx) in enumerate(kf.split(users), 1):
    ...
    # 4. 用支持向量机做分类 + 网格搜索优化超参数
    # (Support Vector Machine with Grid Search for hyperparameter optimization)

    # 定义网格搜索的参数空间 (优化版本 - 快速训练)
    # Define the parameter space for grid search (optimized for speed)
    param_grid = {
        'C': [1, 10],                              # 正则化参数：只测试最常用的值
        'gamma': ['scale', 0.01],                  # RBF核系数：默认值 + 一个常用值
        'kernel': ['rbf']                          # 核函数：只用RBF（对BERT特征效果好）
    }
    
    print(f"    开始网格搜索最优SVM参数（样本数: {len(train_embeddings)}）...")
    print(f"    搜索空间: C={param_grid['C']}, gamma={param_grid['gamma']}, kernel={param_grid['kernel']}")
    
    # 初始化网格搜索 (快速版本)
    # Initialize grid search with 2-fold cross-validation for speed
    grid_search = GridSearchCV(
        SVC(probability=True, random_state=SEED),   # 基础SVM模型
        param_grid,                                 # 参数搜索空间
        cv=2,                                      # 2折交叉验证（快速验证）
...
gamma_values = [params['gamma'] for params in all_best_params]
kernel_values = [params['kernel'] for params in all_best_params]

print(f"C值频率: {dict(Counter(c_values))}")
print(f"gamma值频率: {dict(Counter(gamma_values))}")
print(f"kernel频率: {dict(Counter(kernel_values))}")
...
print("\n===== 网格搜索优化建议 =====")
print("基于以上结果，建议的SVM参数配置:")
c_mode = Counter(c_values).most_common(1)[0][0]
gamma_mode = Counter(gamma_values).most_common(1)[0][0]
kernel_mode = Counter(kernel_values).most_common(1)[0][0]
print(f"推荐配置: C={c_mode}, gamma='{gamma_mode}', kernel='{kernel_mode}'")
...
print(f"\n📱 MacBook Pro 优化建议:")
print(f"   内存: 16GB+ 推荐 (处理大型BERT模型)")
print(f"   存储: SSD 必需 (快速数据加载)")
print(f"   散热: 确保良好通风 (长时间训练)")
if DEVICE.type == 'mps':
